...
class AdminDriver extends Driver {
    ...
    public void acceptTransaksi(int index) {
        if (index >= 0 && index < listTransaksi.size()) {
            ...
            listTransaksi.remove(index); // Remove the accepted transaction from pending list
        ...
        for (Barang transBarang : transaksi.barang) {
            for (Barang barang : listBarang) {
                if (barang.id.equals(transBarang.id)) {
                    barang.quantity -= transBarang.quantity; // Reduce stock
                    break;
                }
            }
        }
        saveAllBarangToFile(); // Save updated stock levels to the file
    }
